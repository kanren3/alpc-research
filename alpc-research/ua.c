...
)
{
    ...
    RtlInitUnicodeString(&Server->PortName, PortName);
    ...
    Server->ServerThreadHandle = CreateThread(NULL, 0, UaServerWorker, Server, 0, NULL);

    if (NULL == Server->ServerThreadHandle) {
        goto Cleanup;
    }
...
)
{
    ...
    RtlInitUnicodeString(&PortName, ServerPortName);
